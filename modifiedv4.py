...
class MerchantDataProcessor:
    def __init__(self, max_workers=10, rate_limit_delay=0.1):
        ...
        # ---------- Defaults (aligned to MPParser expectations) ----------
        
        self.hash_input_mode = os.getenv("HASH_INPUT_MODE", "PLAINTEXT").upper()

        # Field-level protection
        
        self.pii_encryption = os.getenv("PII_ENCRYPTION", "MP_AES_ECB").upper()
        ...
        self._aesgcm = None
        self._aes_ecb_key = None
        if _CRYPTO_AVAILABLE:
            key_bytes = None
            if self.pii_key_hex:
                try:
                    key_bytes = bytes.fromhex(self.pii_key_hex)
                    if len(key_bytes) not in (16, 24, 32):
                        logger.warning("PII_KEY_HEX must be 16/24/32 bytes (32/48/64 hex). Crypto modes may fall back to B64.")
                        key_bytes = None
                except Exception as e:
                    logger.warning(f"Invalid PII_KEY_HEX ({e}). Crypto modes may fall back to B64.")
                    key_bytes = None

            if self.pii_encryption == "AES_GCM":
                if key_bytes and len(key_bytes) in (16, 24, 32):
                    ...
                    logger.warning("AES-GCM selected but no valid key; falling back to B64.")
                    self.pii_encryption = "B64"

            elif self.pii_encryption == "MP_AES_ECB":
                if key_bytes and len(key_bytes) in (16, 24, 32):
                    self._aes_ecb_key = key_bytes
                else:
                    logger.warning("MP_AES_ECB selected but no valid key; falling back to B64.")
                    self.pii_encryption = "B64"
        else:
            if self.pii_encryption in ("AES_GCM", "MP_AES_ECB"):
    ...
    def _load_processing_prefs_from_xml(self, xml_path: str):
        ...
        try:
            tree = ET.parse(found)
            root = tree.getroot()
            # If the XML lists <Hash> then <Encrypt>
            hash_node = any(n.tag.lower().endswith("hash") for n in root.iter())
            encrypt_node = any(n.tag.lower().endswith("encrypt") for n in root.iter())
            if hash_node and encrypt_node:
                self.hash_input_mode = "PLAINTEXT"
    ...
    # === Encryption helpers ===
    def _base64_raw(self, value: str) -> str:
        """Return base64 of the ASCII string, without any 'b64:' label."""
        return base64.b64encode(value.encode()).decode()

    def _encrypt_aes_ecb_pkcs7_to_b64(self, value: str) -> str:
        """
        Deterministic MPParser-like: AES-ECB + PKCS7 padding; base64 encode ciphertext.
        Requires a valid AES key in self._aes_ecb_key.
        """
        if not (_CRYPTO_AVAILABLE and self._aes_ecb_key):
            # Fallback to raw base64
            return self._base64_raw(value)
        padder = padding.PKCS7(128).padder()
        padded = padder.update(value.encode()) + padder.finalize()
        cipher = Cipher(algorithms.AES(self._aes_ecb_key), modes.ECB())
        encryptor = cipher.encryptor()
        ct = encryptor.update(padded) + encryptor.finalize()
        return base64.b64encode(ct).decode()

    def _encrypt_aes_gcm_labeled(self, value: str) -> str:
        if not (_CRYPTO_AVAILABLE and self._aesgcm):
            return self._base64_raw(value)
        nonce = os.urandom(12)
    ...
    def encrypt_value(self, value):
        """Field-level protection per mode."""
        if not value:
            return ''
        if self.pii_encryption == "MP_AES_ECB":
            return self._encrypt_aes_ecb_pkcs7_to_b64(value)
        if self.pii_encryption == "AES_GCM":
            return self._encrypt_aes_gcm_labeled(value)
        # B64
        return self._base64_raw(value)
